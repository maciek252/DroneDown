...
enum DroneDownState {NEUTRAL, OFF, ON};
DroneDownState droneDownState = NEUTRAL;
int droneDownValue = 1000;
...
void checkButton(void) {
	...
	if (digitalRead(BTN) == 0) {     // Check the button
		...
		if (0 == digitalRead(BTN)) {
			...
			if (swapProfile) {
				setDefaultProfile(
						(defaultProfile + 1) % (TX_PROFILE_COUNT + 1));
				setupProfile();
				txReadEeprom();
				delay(500);
				return;
...
void setup(void) {
	...
	/*
	 ...
	 pinMode(PPM_IN, OUTPUT); //PPM from TX
...
//			val = 1020;
		...
		uint8_t i;
		for (i = 0; i < ppmCounter; i++) {
			PPM[i] = droneDownValue;
		}
		ppmAge = 0;
	}

	//Serial.println("ppmCounter=" + ppmCounter);
	// MSz end
