	...
	for (i = 0; i < OUTPUTS; i++) {
		...
		if ((map & 0xf0) == 0x10) { // 16-31
			digitalWrite(OUTPUT_PIN[i],
					(PPM[map & 0x0f] > switchThresholds[i % 3]) ? HIGH : LOW);
		}
	}
}
	...
	while ((!timeout) || ((millis() - start) < timeout)) {
		if (RF_Mode == Received) {
			...
			if (rxb == 'b') {
				...
				for (uint8_t i = 0; i < 16; i++) {
					failsafePPM[i] = ((uint16_t) spiReadData() << 8)
							+ spiReadData();
				}
				rxb = 'G';
				failsafeSave();
...
void setup() {
	...
	// ms test
	//pinMode(RSSI_OUT, OUTPUT); //PPM from TX
...
void checkSerial() {
	while (Serial.available()
			&& (((serial_tail + 1) % SERIAL_BUFSIZE) != serial_head)) {
...
void loop() {
	...
	// enable pullup for TX:s with open collector output

	//1000 - nie bylo wcisniecia przycisku na Tx, on 276 off 76
	if (PPM[1] < 1000 && PPM[1] > 0) {
		if (PPM[1] < 100) {
			droneDownState = 0;
		} else if (PPM[1] > 100)
			droneDownState = 1;
	...
	if (linkAcquired) {
		...
		if (numberOfLostPackets) {
			...
			if ((rx_config.beacon_frequency) && (nextBeaconTimeMs)
				...
				rx_reset();
				nextBeaconTimeMs = (millis()
						+ (1000UL * rx_config.beacon_interval)) | 1; // avoid 0 in time
			}
		}
	} else {
